...
-Plotting - ggplot2 - 'aes' is used to construct aesthetic mappings, while 'geom_boxplot' will plot a box and whiskers chart.  
-Data munging/wrangling - dplyr - 'group_by' is used to group one or more variables, while 'n' will provide the total number of observations in the declared dataset.   
-Reshaping data - tidyr - 'gather' is used to turn column names into actual variables.  We are able to then rename the new columns to something more appropriate.  'spread' is used to collapse multiple rows into a single row, while adding new columns, or keys.  
-Importing/exporting data - readr - 'read_csv' will read a csv file and import the data in RStudio for analysis.  'write_csv' will write the data to a new csv file.
...
1.2 - There are two separate vectors on the chart (Age and Sex), however the chart layout leads the reader to compare there directly against eachother (e.g. Women vs. Under 45)  
1.3 - There is no organization to the color coding.  The Sex vector has separate colors, but the Age vector is a single color.
...
- The number of diamonds above the $10000 price point increases as the cut increases  
- The overall proportion is a fairly small number across the board, since the size is very small which also impacts pricing  
